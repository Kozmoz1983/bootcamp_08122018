...
if DEBUG:
    ...
    if gracz_x == skarb_x and gracz_y == skarb_y:
        print("Wygrales !!")
        print(f"Minimalna liczba krokow wynosila: {min_l_krokow_po_wyl}")
        print(f"Zrobiles krokow: {liczba krokow}")
        break
    ...
    if liczba_krokow >= min_l_krokow_po+wyl * 2:
        skarb_x = randint(1, 10)
        skarb_y = randint(1, 10)
        min_l_krokow_po_wyl = abs(skarb_x - gracz_x) + abs(skarb_y - gracz_y)
        liczba_krokow = 0
        print("Przekroczyles dopuszczlna liczbe krokow. Skarb zmienil polozenie")
